        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalFlags = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFlags">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Flag</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FlagId" type="text" placeholder="Enter FlagId" v-model="flagToAdd.flagId"></base-input>
  <base-input label="Comment" type="text" placeholder="Enter Comment" v-model="flagToAdd.comment"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="flags" :row-key="record => record.FlagId" :pagination="pagination"
...
import FlagPictureView from './FlagPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'flagId',
          visible: true,
          scopedSlots: { customRender: 'flagId' },
          sorter: true
          //sorter: (a, b) => a.flagId - b.flagId,
          //sorter: (a, b) => a.flagId.localeCompare(b.flagId),
        },
        {
          title: 'Comment',
		dataIndex: 'comment',
      ...
      sortBy: 'flagId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFlagsTable() {
      ...
      for (let i = 0; i < this.flags.length; i++) {
        flagsTableData.push({
          id: i,
          flagId: this.flags[i].flagId,
          year: this.flags[i].year,
          date: this.flags[i].date,
          competitionId: this.flags[i].competitionId,
          flagId: this.flags[i].flagId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-flags',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFlagsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalFlags = false;

      const currentDate = new Date().getTime();
      this.flagToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.flagToAdd);
      console.log(jsonData);
      
      const res = await FlagService.addFlag(jsonData);
